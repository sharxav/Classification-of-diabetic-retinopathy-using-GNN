#Reference Page - https://github.com/endrol/DR_GCN/blob/9ad1929910ed30c3a623c25ba0da0198bd1655f5/dr_gcn/engine.py
import os
import shutil
import time
...
class Engine(object):
    ...
    def learning(self, model, criterion, train_dataset, val_dataset, optimizer=None):
        ...
        np.save('checkpoints/eyepacs/loss_recorder_train.npy', self.state['loss_recorder_train'])
        self.state['loss_recorder_val'] = np.array(self.state['loss_recorder_val'])
        np.save('checkpoints/eyepacs/loss_recorder_val.npy', self.state['loss_recorder_val'])

        self.state['accuracy_recorder_train'] = np.array(self.state['accuracy_recorder_train'])
        np.save('checkpoints/eyepacs/accuracy_recorder_train.npy', self.state['accuracy_recorder_train'])
        self.state['accuracy_recorder_val'] = np.array(self.state['accuracy_recorder_val'])
        np.save('checkpoints/eyepacs/accuracy_recorder_val.npy', self.state['accuracy_recorder_val'])

        self.state['kappa_recorder'] = np.array(self.state['kappa_recorder'])
        np.save('checkpoints/eyepacs/kappa_recorder.npy', self.state['kappa_recorder'])
...
class GCNMultiLabelMAPEngine(MultiLabelMAPEngine):
    def on_forward(self, training, model, criterion, data_loader, optimizer=None, display=True):
        ...
        inp_var = torch.autograd.Variable(self.state['input']).float().detach()  # one hot endcoding
